           ...
           HgfsOp op)              // IN: Opcode for file type (file or dir)
{
  ...
  retry:
   ...
   if (HgfsBuildPath(fileName, HGFS_NAME_BUFFER_SIZET(HGFS_PACKET_MAX, reqSize),
                     dentry) < 0) {
   ...
   result = CPName_ConvertTo(fileName,
                             HGFS_NAME_BUFFER_SIZET(HGFS_PACKET_MAX, reqSize),
                             fileName);
   if (result < 0) {
                       ...
                       Bool *changed)         // OUT: Have the attrs changed?
{
   ...
   case HGFS_OP_SETATTR_V2: {
	 ...
	 fileNameLength = &requestV2->fileName.length;
      ...
      reqBufferSize = HGFS_NAME_BUFFER_SIZE(HGFS_PACKET_MAX, requestV2);
   ...
   case HGFS_OP_SETATTR: {
      ...
      fileNameLength = &request->fileName.length;
      reqSize = sizeof *request;
      reqBufferSize = HGFS_NAME_BUFFER_SIZE(HGFS_PACKET_MAX, request);
...
retry:
   ...
   if (HgfsBuildPath(oldName, HGFS_NAME_BUFFER_SIZET(HGFS_PACKET_MAX, reqSize),
                     oldDentry) < 0) {
   ...
   *oldNameLength = result;
   ...
   if (HgfsBuildPath(newName, HGFS_NAME_BUFFER_SIZET(HGFS_PACKET_MAX, reqSize) - result,
                     newDentry) < 0) {
   ...
   result = CPName_ConvertTo(newName,
                             HGFS_NAME_BUFFER_SIZET(HGFS_PACKET_MAX, reqSize) - result,
                             newName);
   if (result < 0) {
