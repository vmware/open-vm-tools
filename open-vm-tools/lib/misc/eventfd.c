 ...
 * MayPeekForEventfd
 *
 *      Is it OK to peek around for eventfd?
 *
 *      The eventfd facility exists officially in Linux 2.6.22 and later.
 *
 *      Peeking around for syscalls has proven highly problematic with older
 *      Linux kernels. Don't even attempt to use it if the kernel isn't
 *      2.6.23 or later (the farthest back that it has been tested and is
 *      known to be working).
 *
 * Results:
 *      TRUE    Yes
...
static Bool
MayPeekForEventfd(void)
{
   if (HostType_OSIsVMK()) {  // Always OK to peek on ESX
      return TRUE;
   }

#if defined(__linux__)
   /*
    * Only OK on Linux 2.6.23 and later
    */

#define LKV(a, b, c) (((a) << 16) + ((b) << 8) + (c))  // kernel version

   if (LKV(Hostinfo_OSVersion(0), Hostinfo_OSVersion(1),
           Hostinfo_OSVersion(2)) < LKV(2, 6, 23)) {
      return FALSE;
   }
