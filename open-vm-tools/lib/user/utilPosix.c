   ...
   if (lim.rlim_cur != RLIM_INFINITY && lim.rlim_cur < fdsDesired) {
	 ...
	 lim.rlim_max = fdsDesired;
      ...
      if (err != 0) {
         ...
         Log("UTIL: Change file descriptor limit from soft %u,hard %u "
             "to soft %u,hard %u.\n",
             (uint32)curFdLimit, (uint32)maxFdLimit,
             (uint32)lim.rlim_cur, (uint32)lim.rlim_max);
      }
   } else {
      Log("UTIL: Current file descriptor limit: soft %u, hard %u.\n",
          (uint32)lim.rlim_cur, (uint32)lim.rlim_max);
   }
   return err;
}
